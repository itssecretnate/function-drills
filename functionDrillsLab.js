...
function nameCheck(name) {
  if(name === "Steve") return 'What is up Steven?';
...
console.log(emailCheck(("natemena153@gmail.com")))
...
function chocoFrogs(gold) {
  let chocolateFrogsPrice = 3;

  return gold / chocolateFrogsPrice;
}

let totalFrogs = chocoFrogs(300);
...
function chocoFrogs2(gold) {
  ...
  return Math.floor(gold / chocolateFrogsPrice);
}

let totalFrogs2 = chocoFrogs2(4);
...
function checkArrayOrder(arr) {
  ...
  for(let i = 0; i < arr.length; i++) {
    if(arr[i+1] - arr[i] < 0) isAscending = false; 
  }
  return isAscending
}

console.log(checkArrayOrder(sampleArray));
